<script lang="ts">
	...
	async function main() {
		...
		const responseId = await fetch(
			new URL(`https://fwd.innopolis.university/api/hw2?email=s.palkina@innopolis.university`)
		);
...
<body>
	<div class="comic-picture">
		{#if info}
			...
			<time
				>{`Uploaded ${formatDistanceToNow(new Date(Date.UTC(Number.parseInt(info.year), Number.parseInt(info.month) - 1)))} ago`}</time
			>
		{/if}
	</div>
</body>
